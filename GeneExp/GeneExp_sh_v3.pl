...
my $python="export LD_LIBRARY_PATH=/ldfssz1/ST_HEALTH/Reference/ST_CANCER/tools/Anaconda/anaconda3/envs/align_splice_fusion_tools/bin:/ldfssz1/ST_HEALTH/Reference/ST_CANCER/tools/Anaconda/anaconda3/envs/align_splice_fusion_tools/lib/python2.7 && $Bin/../Bin_CentOS6/python";
...
foreach my $s (keys %sample) {
	...
	# Mkshell("$shdir/Mapstat_ReadsRandomInGene_$s.sh", $exp_shell);
	...
	# print DEPL "$shdir/GeneExp2_$s.sh:15G:$cpu_num2\t$shdir/Mapstat_ReadsRandomInGene_$s.sh:4G\n";
	push @depends, "$shdir/GeneExp2_$s.sh:15G:$cpu_num2";
	push @depends2, "$shdir/Mapstat_ReadsRandomInGene_$s.sh:4G";
    ...
    # print RES "GeneExp_RSEM\t$shdir/Mapstat_$s.sh\t$mapdir/$s.Bowtie2Gene.MapReadsStat.xls\t1\n";
	...
	Mkshell("$shdir/Mapstat_ReadsRandomInGene_$s.sh", $exp_shell);
	print DEPL "$shdir/GeneExp2_$s.sh:15G:$cpu_num2\t$shdir/Mapstat_ReadsRandomInGene_$s.sh:4G\n";
	print RES "GeneExp_RSEM\t$shdir/Mapstat_ReadsRandomInGene_$s.sh\t$randomdir/$s.ReadsRandom.png\t1\n";
	# $exp_shell .= "rm $resultdir/$s.bam && \\\n"; 
	...
	$exp_shell .= "if [ ! -f $resultdir/$s.depth.txt.gz ];then\n /bin/gzip $resultdir/$s.depth.txt \nfi && \\\n";
	...
	print DEPL "$shdir/GeneExp2_$s.sh:15G:$cpu_num2\t$shdir/ReadsCoverage_$s.sh:4G\n";
    ...
    $exp_shell .= "rm $resultdir/$s.transcript.bam";
	...
	print DEPL "$shdir/GeneExp2_$s.sh:15G:$cpu_num2\t$shdir/SeqSaturation_$s.sh:1G\n";
	print RES "GeneExp_RSEM\t$shdir/SeqSaturation_$s.sh\t$coverdir/$s.SeqSaturation.png\t1\n";
        ...
        print RES "GeneExp_RSEM\t$shdir/GeneExp2_$s.sh\t$fpkmdir/$s.gene.fpkm.xls\t1\n";
        print RES "GeneExp_RSEM\t$shdir/GeneExp2_$s.sh\t$fpkmdir/$s.transcript.fpkm.xls\t1\n";
        print RES "GeneExp_RSEM\t$shdir/Mapstat_ReadsRandomInGene_$s.sh\t$mapdir/$s.Bowtie2Gene.MapReadsStat.xls\t1\n";
        print RES "GeneExp_RSEM\t$shdir/Mapstat_ReadsRandomInGene_$s.sh\t$randomdir/$s.ReadsRandom.png\t1\n";
        print RES "GeneExp_RSEM\t$shdir/ReadsCoverage_$s.sh\t$coverdir/$s.ReadsCoverage.png\t1\n";
        print RES "GeneExp_RSEM\t$shdir/SeqSaturation_$s.sh\t$coverdir/$s.SeqSaturation.png\t1\n";
        # push @depends, "$shdir/GeneExp2_$s.sh:15G:$cpu_num2";
...
$exp_shell .= "if [ -e $outdir/*gene.fpkm.xls ];then\nrm $outdir/*gene.fpkm.xls; \nfi\n";
