...
foreach my $s (keys %sample) {
	...
	print DEPL "$sample{$s}[-1]\t$shdir/GeneExp1_$s.sh:30G:$cpu_num1\n";
	...
	if (@p>1){
	print DEPL "$shdir/build_index.sh:0.5G\t$shdir/GeneExp1_$s.sh:200G:$cpu_num1\n";
	}
...
#	# $exp_shell .= "rm $resultdir/$s.transcript.sorted.bam $resultdir/$s.bam $resultdir/$s.transcript.bam && \\\n";
	...
	$exp_shell .= "mv $resultdir/$s.SeqSaturation* $saturationdir && \\\n";
        $exp_shell .= "rm $resultdir/$s.transcript.sorted.bam $resultdir/$s.bam $resultdir/$s.transcript.bam $resultdir/$s.transcript.sorted.bam.bai";
	...
	# print RES "GeneExp_RSEM\t$shdir/SeqSaturation_$s.sh\t$coverdir/$s.SeqSaturation.png\t1\n";
        Mkshell("$shdir/GeneExp2_$s.sh", $exp_shell);
        print DEPL "$shdir/GeneExp1_$s.sh:200G:$cpu_num1\t$shdir/GeneExp2_$s.sh:200G:$cpu_num2\n";
        print DOWL "$s\t$resultdir/$s.gene.fpkm.xls\t$shdir/GeneExp2_$s.sh:200G:$cpu_num2\n";
        print RES "GeneExp_RSEM\t$shdir/GeneExp2_$s.sh\t$fpkmdir/$s.gene.fpkm.xls\t1\n";
        print RES "GeneExp_RSEM\t$shdir/GeneExp2_$s.sh\t$fpkmdir/$s.transcript.fpkm.xls\t1\n";
        print RES "GeneExp_RSEM\t$shdir/GeneExp2_$s.sh\t$mapdir/$s.Bowtie2Gene.MapReadsStat.xls\t1\n";
        ...
        push @depends, "$shdir/GeneExp2_$s.sh:200G:$cpu_num2";
