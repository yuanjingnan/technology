...
size=ifelse((len==1|len==2|len==3),5,36/len)
label_size=ifelse((len==1|len==2|len==3),15,96/len)
...
geom_text(aes(label = GeneNumber, y = pos), size = size)+
...
theme(axis.text.x = element_text(size=label_size,angle=30, hjust =1),panel.grid =element_blank())+
