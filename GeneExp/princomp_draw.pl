...
=head1 Example
  ...
  eg 2: perl princomp_draw.pl --expr expression.xls --group groupinfo.txt -o workdir
...
$Rscript = "unset R_LIBS_USER && unset R_LIBS && \\\n unset R_LIBS_SITE && \\\n export R_LIBS_USER=\"/ldfssz1/ST_CANCER/CGR/SHARE/tools/Anaconda/anaconda2/envs/r-3.5.1/lib/R/library\" && $Bin/../Bin_CentOS6/R/Rscript" unless ($Rscript and -f $Rscript);
my $convert = "$Bin/../Bin_CentOS6/convert ";
...
if ($Group_info) {
	system "sed '1s/-/_/g' $Expression_file >$Expression_file.tmp";
	$Expression_file = "$Expression_file.tmp";
...
print RSC <<EOF;
expr <- read.table("$Expression_file",header = TRUE,check.names=F)
options(width = 160)
expr.dim <- dim(expr)
N_samp <- expr.dim[2] - 1
N_gene <- expr.dim[1]
...
pdf("$Outdir/PCA.Com1-Com2.pdf", 7, 7)
...
#  labs(x='Component 1', y='Component 2')+
  ...
  theme(panel.background = element_rect(fill='transparent'),panel.grid=element_line(color='grey'),panel.border=element_rect(fill='transparent',color='black'),axis.title=element_text(size=15) )+theme(legend.key.size= unit(0.35, "cm"))
dev.off()

pdf("$Outdir/PCA.Com3-Com4.pdf", 7, 7)
...
#system("conver e-density 100  $Outdir/PCA.exp.xls.3D.pdf $Outdir/PCA.exp.xls.3D.png");
...
system("$convert -density 120 $Outdir/PCA.Com1-Com2.pdf $Outdir/PCA.Com1-Com2.png");
